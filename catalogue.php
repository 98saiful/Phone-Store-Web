...
<html lang="en">
  <head>
    ...
    <title>Google Store : Catalogue</title>
    <link rel="icon" href="logor.jpg" type="image/ico" sizes="40x40">
...
<div class="container-fluid"> 
  ...
  <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-10 col-md-offset-1">
      <div class="page-header">
        <h2 style="font-family: 'Segoe UI';">Catalogue</h2>
      </div>
      ...
      <form action="catalogue.php" method="get" >
            ...
            <!-- /input-group -->
              ...
              <div class="btn-group" style="width: 100%;">
                ...
                <?php try {
                  ...
                  $stmt = $conn->prepare("SELECT DISTINCT fld_product_brand FROM tbl_products_a166118_pt2");
          ...
          if(isset($_GET['search'])) {
                        ...
                        if ($getSearch == "all") {
                          ...
                          $stmt = $conn->prepare("SELECT * FROM tbl_products_a166118_pt2 WHERE fld_product_name LIKE '%".$getSearch."%'");
                        }

                      } else if(isset($_GET["category"])){
                        $getCategory = $_GET["category"];
                        $stmt = $conn->prepare("SELECT * FROM tbl_products_a166118_pt2 WHERE fld_product_brand = \"".$getCategory."\"");

                      } else if (isset($_GET["sort"])) {
                        $getSort = $_GET["sort"];

                        if($getSort == "nameAsc"){
                          $stmt = $conn->prepare("SELECT * FROM tbl_products_a166118_pt2 ORDER BY fld_product_name ASC");
                        } elseif ($getSort == "nameDesc") {
                          $stmt = $conn->prepare("SELECT * FROM tbl_products_a166118_pt2 ORDER BY fld_product_name DESC");
                        } elseif ($getSort == "priceAsc") {
                          $stmt = $conn->prepare("SELECT * FROM tbl_products_a166118_pt2 ORDER BY fld_product_price ASC");
                        } elseif ($getSort == "priceDesc") {
                          $stmt = $conn->prepare("SELECT * FROM tbl_products_a166118_pt2 ORDER BY fld_product_price DESC");
                        } 
                      } else {
                        $stmt = $conn->prepare("SELECT * FROM tbl_products_a166118_pt2 LIMIT $start_from, $per_page");
                      }
        ...
        <?php foreach($result as $readrow) { ?>
              ...
              <?php if ($readrow['fld_product_image'] == "" ) { ?>
                <img src="products/nophoto.png" class="img-responsive">
              <?php } else { ?>
              ...
              <a href="products_details.php?pid=<?php echo $readrow['fld_product_num']; ?>" class="btn btn-primary btn-block" role="button">View</a>
            </div>
            <!-- /.thumbnail -->
                    </div>
        <?php } $conn = null; ?>
